...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    # end if
            ...
            # at this point   can be sure the list of possible plates has at least one plate
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        # clear the image more removing all the unknown noise from it.
        if Start.showSteps == True: 
            Image.fromarray(possiblePlate.imgThresh).show()
            # input('Press Enter to Continue....')

                # find all possible chars in the plate,
                # finds all contours, then only include contours that could be chars (without comparison to other chars yet)
    ...
    return listOfPossiblePlates_refined # return the list of plates with the probable plate number of each plate.
...
def checkIfPossibleChar(possibleChar):
            ...
            # note that   are not (yet) comparing the char to other chars to look for a group
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
            # re-arrange the one big list of chars into a list of lists of matching chars,
...
def findListOfMatchingChars(possibleChar, listOfChars):
    ...
    for possibleMatchingChar in listOfChars:                # for each char in big list
        if possibleMatchingChar == possibleChar:    
            # dont add same character itself in the list 
            continue                                
...
# Pythagoras' theorem to calculate distance between two chars
...
def angleBetweenChars(firstChar, secondChar):
    ...
    if fltAdj != 0.0:                           # check to make sure   do not divide by zero if the center X positions are equal, float division by zero will cause a crash in Python
...
# 2 chars overlapping or too close to each other to possibly be separate chars, remove the inner (smaller) char,
# multiple contours for same char, but only include the char once
